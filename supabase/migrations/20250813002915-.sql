...
CREATE POLICY "Users can manage own whatsapp_auth"
...
CREATE POLICY "Users can view own whatsapp_status"
ON public.whatsapp_status
FOR SELECT
TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "Users can upsert own whatsapp_status"
ON public.whatsapp_status
FOR INSERT
TO authenticated
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own whatsapp_status"
ON public.whatsapp_status
FOR UPDATE
TO authenticated
USING (auth.uid() = user_id)
...
CREATE TRIGGER update_whatsapp_status_updated_at
